    ...
    @foreach($list->menuParentItems as $item)
        <li class="treeview {{active_link_sub($item->url)}}">
            <a href="{{$item->url}}">
                ...
                @if($item->subs->count())  <span class="pull-right-container"><i
                        class="fa fa-angle-left pull-right"></i></span>    @endif
            </a>
            @if($item->subs->count())
                <ul class="treeview-menu">
                    ...
                    @foreach($item->subs as $sub_menu)
                        ...
                        <li class="{{active_link_with_class($sub_menu->url,'active')}}"><a href="{{$sub_menu->url}}">
                                ...
                                @if($sub_menu->subs->count())  <span class="pull-right-container"><i
                                        class="fa fa-angle-left pull-right"></i></span>    @endif
                            </a>
                            @if($sub_menu->subs->count())
                                <ul class="treeview-menu">
                                    @foreach($sub_menu->subs as $sub)
                                        <li class="{{active_link_with_class($sub->url,'active')}}"><a
                                                href="{{$sub->url}}">
